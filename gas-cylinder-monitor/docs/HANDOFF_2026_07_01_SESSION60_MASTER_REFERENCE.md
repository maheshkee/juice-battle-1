...
| UNINSTALLED / CYLINDER_ABSENT redesign | DESIGNED, NOT IMPLEMENTED — blocked on product decision |
...
| UNINSTALLED / CYLINDER_ABSENT redesign | **DESIGNED, blocked on product decision** | See Part 13 for design; needs: typical cylinder-removal-duration answer | Hub-side (`domain.py`) once unblocked — not firmware |
...
### UNINSTALLED / CYLINDER_ABSENT redesign — design locked, implementation blocked (Session 61)
...
**The proposed design** (combining both ideas raised — weight-matching and explicit button):
```
TRACKING
    ↓ weight drops (cylinder lifted) — record lift_weight_g
CYLINDER_ABSENT          ← new intermediate state, replaces immediate timeout logic
    ↓ weight returns, matches lift_weight_g within noise floor
...
INSTALL FLOW             ← different cylinder detected — fresh anchor sequence
    ↓ explicit uninstall button pressed (anytime, user-initiated)
UNINSTALLED              ← the only fully intentional path
    ↓ very long timeout safety net (hours, not 120s)
UNINSTALLED              ← failsafe only, for a cylinder that never returns
```
...
**What's blocking implementation**: one product decision — is the common removal scenario
a quick lift (clean, move, shake — seconds to a minute) or an extended absence (refill,
lend, swap — hours)? This determines tuning for the `CYLINDER_ABSENT` state's own internal
timing and whether the weight-match tolerance needs adjustment for thermal drift over
longer absences. Raised twice now (2026-06-29 and 2026-07-02) without an answer — needs
resolving at the start of Session 62 before any code gets written.

**Where it lands when unblocked**: `hub/python/domain.py`. Hub-side only — no firmware
