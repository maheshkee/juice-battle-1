// 3E001 cal_factor v5.2 | 3-cell platform | self-characterising | per-iteration weight entry | timing
...
void setup() {
  ...
  Serial.println("3E001 cal_factor v5.2 | 3-cell platform | self-characterising | per-iteration weight entry | timing");
  Serial.println("Keep platform EMPTY and undisturbed until [TARE] appears. You will enter weight each iteration.");
...
void loop() {
  ...
  if (phase == PHASE_MEASURE_GATE) {
    ...
    if (gate_step == 1) {
      // Per-iteration weight entry, then placement gate
      do {
        Serial.println("Enter weight of object now on platform (grams), press Enter:");
        while (Serial.available()) Serial.read();
        {
          char wbuf[32];
          uint8_t wi = 0;
          while (true) {
            if (Serial.available()) {
              char c = Serial.read();
              if (c == '\n' || c == '\r') break;
              if (wi < sizeof(wbuf) - 1) wbuf[wi++] = c;
            }
          }
          wbuf[wi] = '\0';
          ref_weight_g = atof(wbuf);
        }
        if (ref_weight_g < 50.0f || ref_weight_g > 5000.0f) {
          Serial.println("Invalid weight. Enter a value between 50 and 5000 grams.");
        }
      } while (ref_weight_g < 50.0f || ref_weight_g > 5000.0f);
      char buf[48];
