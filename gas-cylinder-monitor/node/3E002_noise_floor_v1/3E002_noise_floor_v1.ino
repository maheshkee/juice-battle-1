// 3E-002 Run A - BLE OFF (hardware baseline noise)
...
static void handleStabilising(long raw) {
    ...
    if (pass) {
        ...
        if (s1_consec >= STAB_CONSEC_REQ) {
            ...
            Serial.println("[P2] Collecting 200 valid samples (BLE OFF)...");
...
static void handleNoiseCapture(long raw) {
    ...
    Serial.println();
    Serial.println("  === 3E-002 NOISE FLOOR RESULT (BLE OFF) ===");
    char buf[80];
...
void setup() {
    ...
    Serial.println("3E-002 Run A - BLE OFF - noise floor characterisation");
