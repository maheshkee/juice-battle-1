// 3E-003 BLE transport v1 - ESP32-C3 sends {grams, quality, sigma} to hub
// 3-cell YZC-161A parallel | cal_factor=36.1 raw/g LOCKED 2026-06-12
...
// --- State machine ---
enum Phase { PHASE_SETTLING, PHASE_STABILISING, PHASE_NOISE_CAPTURE, PHASE_RUNNING };
...
static float         noise_threshold_g = 0.0f;  // set at end of handleNoiseCapture()
static float         tare_raw_g        = 0.0f;  // set at end of handleStabilising() as float
...
// Advertising NOT started here - deferred to end of Phase 2.
// =================================================================
...
// BLE advertising starts at end of this phase, after noise is locked.
// =================================================================
static void handleNoiseCapture(long raw) {
    ...
    // BLE advertising starts here - noise floor locked, ready for production
...
// =================================================================
// Quality computation - copied verbatim from E003_ble_transport.ino
// =================================================================
...
// =================================================================
// Phase 3 - running: collect, compute quality, send BLE notify
// millis() pacing - all timing internal, ignores raw passed from loop()
// =================================================================
static void handleRunning() {
    if (millis() - lastSend < LOOP_INTERVAL_MS) return;

    // Collect LOOP_SAMPLES valid reads into float array
    // Initialise all slots to (float)LONG_MIN - sentinel for computeQuality()
    ...
    while (collected < LOOP_SAMPLES && attempts < LOOP_SAMPLES * 4) {
        // Wait for HX711 conversion ready (DOUT goes LOW) — max 200ms
        ...
        long raw = readHX711();
        attempts++;
        if (raw == 0L) continue;   // 0L is the corrupt sentinel from readHX711()
        rawSamples[collected++] = (float)raw;
    }

    float out_grams = 0.0f, out_sigma = 0.0f;
    // CAL_FACTOR_LOCKED = 36.1f - LOCKED 2026-06-12, 3-cell parallel, shared plate
    String quality = computeQuality(rawSamples, LOOP_SAMPLES,
                                    tare_raw_g, CAL_FACTOR_LOCKED,
                                    noise_threshold_g, &out_grams, &out_sigma);

    // Build JSON - dtostrf for float formatting, snprintf for assembly
    ...
    if (deviceConnected) {
        ...
        Serial.println("No hub - discarded: " + String(jsonBuf));
    }
    lastSend = millis();
}
...
void setup() {
    ...
    // startAdvertising() is called inside handleNoiseCapture() after noise floor locked

    Serial.println("[BLE] GATT server ready. Advertising deferred until Phase 2 complete.");
