#!/bin/bash
# =============================================================================
# $APP_NAME -- One-time setup script
...
mkdir -p "$HOME/.config/autostart"
cat > "$HOME/.config/autostart/$APP_NAME-launcher.desktop" << EOF
[Desktop Entry]
Type=Application
...
# -- 11. Set $APP_NAME as default app -----------------------------------
...
alias yt-restart='arduino-app-cli app stop user:$APP_NAME && rm -rf $APP_DIR/.cache && arduino-app-cli app start user:$APP_NAME'
...
# -- 15b. Run BT audio setup --------------------------------------------------
log "Running Bluetooth audio setup..."
if [ -f "$APP_DIR/setup_for_bt_audio.sh" ]; then
    bash "$APP_DIR/setup_for_bt_audio.sh"
else
    warn "setup_for_bt_audio.sh not found -- run it manually after reboot"
fi
