...
# title fetching moved to phone side
...
def _push_full_status_to_phone():
    # Send in small chunks to avoid BLE MTU limit
    ...
    push_to_phone('connected_devices', {'devices': [
        {'mac': mac, 'name': d.get('name', mac), 'characteristics': [
            {'uuid': u, 'name': c2.get('name', u[:8]),
             'value': c2.get('value',''), 'flags': c2.get('flags',[])}
            for u, c2 in d.get('characteristics', {}).items()]}
