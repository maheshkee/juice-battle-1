...
def _play_thinking():
    ...
    try:
        if _launcher_send:
            _launcher_send('speak:processing')
    except Exception as e:
        print(f'[VOICE] Thinking sound failed: {e}', flush=True)
...
def _parse_command(text: str):
    ...
    if 'resume' in words or 'play' in words:
...
def _transcribe(audio: np.ndarray) -> str:
    """Transcribe audio using preloaded Moonshine transcriber."""
    if _moonshine_model is None or len(audio) < 1600:
        return ''
    try:
        ...
        class _Listener(TranscriptEventListener):
            def on_line_completed(self, event):
                result_text.append(event.line.text)

        listener = _Listener()
        _moonshine_model.add_listener(listener)
        t0 = time.time()
        _moonshine_model.start()
        t1 = time.time()
        _moonshine_model.add_audio(audio_f32, 16000)
        t2 = time.time()
        _moonshine_model.stop()
        t3 = time.time()
        _moonshine_model.remove_listener(listener)
        print(f'[VOICE] TIMING start={t1-t0:.3f}s add_audio={t2-t1:.3f}s stop={t3-t2:.3f}s', flush=True)
...
def voice_assistant_loop(voice_queue, launcher_send_fn, handle_cmd_fn, set_voice_flag_fn=None):
    ...
    global _moonshine_model, _ww_runner

    _voice_queue   = voice_queue
    _launcher_send = launcher_send_fn

    print('[VOICE] Loading Moonshine tiny streaming model...', flush=True)
    try:
        from moonshine_voice import Transcriber, ModelArch
        _moonshine_model = Transcriber(
            model_path=MOONSHINE_MODEL_PATH,
            model_arch=ModelArch.TINY_STREAMING,
            options={'identify_speakers': 'false', 'return_audio_data': 'false', 'max_tokens_per_second': '4.0'},
        )
        print('[VOICE] Moonshine ready', flush=True)
    except Exception as e:
        print(f'[VOICE] Moonshine load failed: {e}', flush=True)
        return
